...
# ---------------------------------------------------------------------------
# DB — გლობალური სახელები, lifespan-ში ინიციალიზდება
# ---------------------------------------------------------------------------
mongo_client: AsyncIOMotorClient = None  # type: ignore
db = None

# ---------------------------------------------------------------------------
# Object-storage — async, httpx
# ---------------------------------------------------------------------------
storage_key: Optional[str] = None
_storage_lock = asyncio.Lock()  # ← asyncio.Lock, არა threading.Lock
...
async def init_storage() -> str:
    ...
    async with _storage_lock:
        if storage_key:          # double-checked locking
            return storage_key
...
# ---------------------------------------------------------------------------
# Lifespan — DB კავშირი იხსნება/იხურება სწორად
# ---------------------------------------------------------------------------
@asynccontextmanager
...
async def send_verification_email(recipient: str, code: str, purpose: str) -> bool:
    ...
    <table width="100%" cellpadding="0" cellspacing="0"
           style="background:#0a0a20;padding:40px 16px;font-family:'Segoe UI',Arial,sans-serif;">
      <tr><td align="center">
        <table width="480" cellpadding="0" cellspacing="0"
               style="max-width:480px;background:#16162a;border:1px solid #262645;
          ...
          <tr><td style="padding:32px;">
            ...
            <div align="center"
                 style="background:linear-gradient(135deg,#f97316,#ea580c);border-radius:10px;
                        padding:18px;color:#fff;font-size:34px;font-weight:800;letter-spacing:10px;">
              {code}
            </div>
            <div style="color:#94a3b8;font-size:12px;padding-top:24px;line-height:1.6;">
              კოდი მოქმედია 10 წუთის განმავლობაში.
            </div>
          </td></tr>
...
async def register(request: Request, data: RegisterInput):
    ...
    await db.notifications.insert_one({
        "id": str(uuid.uuid4()), "user_id": "admin",
        "text": f"ახალი მომხმარებელი: {data.name} ({email})",
        "created_at": now,
    })
    company_id = None
    if role == "company":
...
# ---------------------------------------------------------------------------
# Admin / Ads / Notifications / Countries / Support / Reviews
# ---------------------------------------------------------------------------
...
async def get_companies_countries():
    countries = await db.companies.distinct("country", {"country": {"$ne": ""}})
    return countries or ["საქართველო"]
...
async def get_user_notifications(user: dict = Depends(get_current_user)):
    ...
    ).sort("created_at", -1).to_list(50) or []
...
# ── Chat read / typing  (ორი route ცალ-ცალკე function-ად) ──────────────────
...
async def chat_mark_read_company(
    company_id: str, request: Request, user: dict = Depends(get_current_user)
):
    body      = await request.json()
    sender_id = body.get("sender_id")
...
async def _upload_to_storage(
    user_id: str, file: UploadFile, allowed_exts: set, max_size: int
) -> tuple[str, str, str]:
    ext = (file.filename.rsplit(".", 1)[-1] if "." in file.filename else "").lower()
...
async def _create_verification(
    user_id: str, field: str, new_value: str, deliver_to: str, purpose: str
) -> bool:
    code = gen_code()
    await db.verifications.delete_many({"user_id": user_id, "field": field})
    await db.verifications.insert_one({
...
async def request_email_change(
    request: Request, data: ContactChangeRequest, user: dict = Depends(get_current_user)
):
...
async def confirm_change(
    request: Request, data: CodeConfirm, user: dict = Depends(get_current_user)
):
...
async def get_company_chat_messages(company_id: str, user: dict = Depends(get_current_user)):
    return await db.messages.find({
        "$or": [
            {"company_id": company_id, "sender_id":    user["id"]},
...
async def send_company_chat_message(
    company_id: str, request: Request, user: dict = Depends(get_current_user)
):
    body    = await request.json()
    text    = (body.get("text") or "").strip()
...
async def post_media_comment(
    company_id: str, media_id: str, request: Request, user: dict = Depends(get_current_user)
):
    body      = await request.json()
    text      = (body.get("text") or "").strip()
