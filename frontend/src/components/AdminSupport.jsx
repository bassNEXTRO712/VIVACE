...
export default function AdminSupport() {
  ...
  return (
    <div className="bg-card border border-border rounded-xl overflow-hidden grid grid-cols-1 md:grid-cols-3 h-[520px] min-h-[520px] max-h-[520px]">
      <div className={`border-r border-border overflow-hidden ${active ? "hidden md:block" : "block"}`}>
        ...
        {convos.length === 0 ? (
          ...
          <div className="divide-y divide-border overflow-y-auto h-[calc(520px-57px)]">
            {convos.map((c) => {
              const uid = c.user_id;
              return (
                <button
      ...
      <div className={`md:col-span-2 flex flex-col h-full min-h-0 ${active ? "flex" : "hidden md:flex"}`}>
        {!active ? (
